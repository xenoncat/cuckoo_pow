...
//4 4 42 50 62 334 (nonce=48)
static uint64_t k0def = 0x777cea935ae4e34f;
static uint64_t k1def = 0x09a08bb1617bdd2e;
...
static void TrimEdgeIter4(void * const context, void * const dst, void * const src, const uint32_t tid, const uint32_t iter)
{
    void * const ctx = __builtin_assume_aligned(context, 64);
    uint32_t * const pu32dst = __builtin_assume_aligned(dst, 64) + B_BUCKETS * 4 * tid;
    void * const pvdstbody = __builtin_assume_aligned(dst, 64) +  B_BUCKETS * THREADS * 4;
...
static void TrimEdgeIter7(void * const context, void * const dst, void * const src, const uint32_t tid, const uint32_t iter)
{
    void * const ctx = __builtin_assume_aligned(context, 64);
    uint32_t * const pu32dst = __builtin_assume_aligned(dst, 64) + B_BUCKETS * 4 * tid;
    void * const pvdstbody = __builtin_assume_aligned(dst, 64) +  B_BUCKETS * THREADS * 4;
    ...
    rdtsc0 = __rdtsc();
    d0 = B7_SLOTS * B7_SLOTSIZE * tid;
    for (d1=0; d1<B_BUCKETS; d1++) {
        pu32dst[d1] = d0;
        d0 += THREADS*B7_SLOTS*B7_SLOTSIZE;
    }
    q2 = 0;     //Technically needs tid here, but 5-byte slots truncates it anyway
    bigbucket = tid * BPT;
    bigbucketend = bigbucket + BPT;
    ...
    for (; bigbucket<bigbucketend; bigbucket++) {
        ...
        for (smallbucket=0; smallbucket<S_BUCKETS; smallbucket++) {
            ...
            while ((void*) pu64readsmall < pvlimit) {
                ...
                q1 |= q2;
                *(uint64_t*) (pvdstbody + pu32dst[d0]) = q1;
                pu32dst[d0] += (pu8deg[d1]>1) ? B7_SLOTSIZE : 0;
                pu64readsmall = ((void*)pu64readsmall) + S1_SLOTSIZE;
            }
            if (unlikely(d3 != 0xfc)) {
                printf("Error It7s %x\n", d3);
    ...
    if (tid==0) {
        ...
        for (d0=0; d0<B_BUCKETS; d0++) {
            for (d1=0; d1<THREADS; d1++) {
                ...
                edgecount += (pu32dst[d3]-d2);
                dbgctr = ((pu32dst[d3]-d2) > dbgctr) ? (pu32dst[d3]-d2) : dbgctr;
                d2 += B7_SLOTS*B7_SLOTSIZE;
            }
        }
        edgecount /= B7_SLOTSIZE;
        dbgctr /= B7_SLOTSIZE;
        printf("Iteration %d: %d; max: %d, rdtsc: %ld\n", iter, edgecount, dbgctr, rdtsc1-rdtsc0);
    }
#endif
...
static void TrimEdgeIter8(void * const context, void * const dst, void * const src, const uint32_t tid, const uint32_t iter)
{
    void * const ctx = __builtin_assume_aligned(context, 64);
    uint32_t * const pu32dst = __builtin_assume_aligned(dst, 64) + B_BUCKETS * 4 * tid;
    void * const pvdstbody = __builtin_assume_aligned(dst, 64) +  B_BUCKETS * THREADS * 4;
    ...
    bigbucketend = bigbucket + BPT;
    pu32big = pvbighead;
    pvbig = pvbigbody + bigbucket * (THREADS*B7_SLOTS*B7_SLOTSIZE);
    d0 = (bigbucket!=0) ? (pu32big[(THREADS-1)*B_BUCKETS+bigbucket-1] + B7_SLOTSIZE*2) : 0;
    pvlimit = pvbigbody + d0;
    pu64readbig = (pvlimit > pvbig) ? pvlimit : pvbig;
    ...
    for (; bigbucket<bigbucketend; bigbucket++) {
        ...
        for (fromthread=0; fromthread<THREADS*B_BUCKETS; fromthread+=B_BUCKETS) {
            pvlimit = pvbigbody + pu32big[fromthread+bigbucket];
            pvbig += B7_SLOTS*B7_SLOTSIZE;  //pvbig is now the beginning of the next bucket
            pvlimit2 = (pvbig<pvlimit) ? pvbig : pvlimit;
            while ((void*) pu64readbig < pvlimit2) {
                q0 = *pu64readbig;
                d0 = q0 & S_BUCKETMASK;
                q1 = q0 & 0xffc000000000;
                q4 += (q1 < q3) ? 0x10000000000 : 0;
                q3 = q1;
                d1 = pu32small[d0];
                *(uint64_t*) (pvsmall0+d1) = q4 | ((q0>>S_LOG)&0xffffffffff);
                pu32small[d0] += S8_SLOTSIZE;
                pu64readbig = ((void*)pu64readbig) + B7_SLOTSIZE;
            }
            pvlimit += B7_SLOTSIZE*2;       //Two more spaces because 8-byte writes were used to write 5-byte slots
            pu64readbig = (pvlimit > pvbig) ? pvlimit : pvbig;  //handle big bucket overflow
        }
        if (unlikely(q4 != 0x3f0000000000)) {
            printf("Error Iter9 %lx\n", q4);
            *pu32abort = 1;
            goto _GiveUpIt9;
        }
        pvsmall = pvsmall0 + S_BUCKETS*4;
        ...
        for (smallbucket=0; smallbucket<S_BUCKETS; smallbucket++) {
            ...
            while ((void*) pu32readsmall < pvlimit) {
                pu8deg[*pu32readsmall & (R_ENTRIES-1)]++;
                pu32readsmall = ((void*)pu32readsmall) + S8_SLOTSIZE;
            }
            while ((void*) pu64readsmall < pvlimit) {
                q0 = *pu64readsmall;
                d0 = (q0>>38) & 0xff;
                ...
                *(uint64_t*) (pvdstbody + pu32dst[d0]) = q1;
                pu32dst[d0] += (pu8deg[d1]>1) ? B7_SLOTSIZE : 0;
                pu64readsmall = ((void*)pu64readsmall) + S8_SLOTSIZE;
            }
            pvsmall += S8_SLOTS*S8_SLOTSIZE;    //advance to the next bucket
            pvlimit += S8_SLOTSIZE*2;           //Two more spaces because 8-byte writes were used to write 5-byte slots
            pu32readsmall = (pvlimit > pvsmall) ? pvlimit : pvsmall;    //handle small bucket overflow
            q2 += (1ULL<<38);
        }
    }
...
static uint32_t TrimRename2(void * const context, void * const dst, void * const src, const uint32_t tid, const uint32_t iter)
{
    ...
    rdtsc0 = __rdtsc();
    d0 = BS_SLOTS * BS_SLOTSIZE * tid;
    for (d1=0; d1<B_BUCKETS; d1++) {
        pu32dst[d1] = d0;
        d0 += THREADS*BS_SLOTS*BS_SLOTSIZE;
    }
    bigbucket = tid * BPT;
    bigbucketend = bigbucket + BPT;
    ...
    for (; bigbucket<bigbucketend; bigbucket++) {
        ...
        for (fromthread=0; fromthread<THREADS*B_BUCKETS; fromthread+=B_BUCKETS) {
            ...
            while ((void*) pu64readbig < pvlimit2) {
                ...
                d0 = q0 & S_BUCKETMASK;
                d1 = pu32small[d0];
                *(uint64_t*) (pvsmall0+d1) = (q0 >> S_LOG); //Mask 0x3fffffffff omitted
                pu32small[d0] += S1_SLOTSIZE;
    ...
    if (tid==0) {
        ...
        for (d0=0; d0<B_BUCKETS; d0++) {
            for (d1=0; d1<THREADS; d1++) {
                ...
                edgecount += (pu32dst[d3]-d2);
                dbgctr = ((pu32dst[d3]-d2) > dbgctr) ? (pu32dst[d3]-d2) : dbgctr;
                d2 += BS_SLOTS*BS_SLOTSIZE;
            }
        }
        edgecount /= BS_SLOTSIZE;
        dbgctr /= BS_SLOTSIZE;
        printf("Iteration %d: %d; max: %d, rdtsc: %ld\n", iter, edgecount, dbgctr, rdtsc1-rdtsc0);
        d0 = 0;
        for (d1=0; d1<THREADS; d1++) {
...
static void TrimEdge1Stage(void * const context, void * const dst, void * const src, const uint32_t tid, const uint32_t ramsize, const uint32_t iter)
{
    ...
    pvsrc = pvsrcbody + srcbucket * (THREADS*BS_SLOTS*BS_SLOTSIZE);
    pu32src = pvsrchead;
    d3 = srcbucket << 15;
    d0 = (srcbucket!=0) ? (pu32src[(THREADS-1)*B_BUCKETS+srcbucket-1] + BS_SLOTSIZE*2) : 0;
    ...
    for (; srcbucket<srcbucketend; srcbucket++) {
        ...
        for (fromthread=0; fromthread<THREADS*B_BUCKETS; fromthread+=B_BUCKETS) {
            ...
            pvsrc += BS_SLOTS*BS_SLOTSIZE;  //pvsrc is now the beginning of the next bucket
            pvlimit2 = (pvsrc<pvlimit) ? pvsrc : pvlimit;
            while ((void*) pu32readsrc < pvlimit2) {
                pu8deg[*pu32readsrc & 0x7fff]++;
                pu32readsrc = ((void*)pu32readsrc) + BS_SLOTSIZE;
            }
            pvlimit += BS_SLOTSIZE*2;       //Two more spaces because 8-byte writes were used to write 5-byte slots
            pu32readsrc = (pvlimit > pvsrc) ? pvlimit : pvsrc;  //handle bucket overflow
        }
        
        for (fromthread=0; fromthread<THREADS*B_BUCKETS; fromthread+=B_BUCKETS) {
            pvlimit = pvsrcbody + pu32src[fromthread+srcbucket];
            pvsrc2 += BS_SLOTS*BS_SLOTSIZE;  //pvsrc2 is now the beginning of the next bucket
            ...
            while ((void*) pu64readsrc < pvlimit2) {
                ...
                *(uint64_t*) (pvdstbody + pu32dst[d2]) = (((uint64_t) d0) << 23) | d3 | d1;
                pu32dst[d2] += (pu8deg[d0] > 1) ? BS_SLOTSIZE : 0;
                pu64readsrc = ((void*)pu64readsrc) + BS_SLOTSIZE;
            }
            pvlimit += BS_SLOTSIZE*2;       //Two more spaces because 8-byte writes were used to write 5-byte slots
...
static void TrimPairsST(void * const context)
{
    ...
    for (outerloop=0; outerloop<30; outerloop++) {
        for (innerloop=0; innerloop<26; innerloop++) {
            moreiter = 0;
            for (fromthread=0; fromthread<THREADS; fromthread++) {
...
static void RecoverSolution(void * const context, uint32_t * listv, uint32_t halfcyclelen)
{
    ...
    for (cyclepos=0; cyclepos<halfcyclelen; cyclepos++) {
        ...
        sipvnode = ((d2 << 12) & 0x7ffff000) | ((d2 >> 11) & 0x00000f00) | d4;
...
void* worker(void *tctx)
{
    ...
    pvbig = pvbig0 + B0_SLOTS * B0_SLOTSIZE * tid;
    ppbig = ctx + CK_BIG0HEAD + B_BUCKETS * 8 * tid;
    for (d1=0; d1<B_BUCKETS; d1++) {
        ppbig[d1] = pvbig;
        pvbig += THREADS*B0_SLOTS*B0_SLOTSIZE;
    }
    ...
    for (d2=0; d2<HALFSIZE/(4*THREADS); d2++) {
        ...
        *(uint64_t*) (ppbig[d0]) = _mm256_extract_epi64(v0, 0);
        ppbig[d0] += B0_SLOTSIZE;
        
        d0 = _mm256_extract_epi32(v1, 2);
        *(uint64_t*) (ppbig[d0]) = _mm256_extract_epi64(v0, 1);
        ppbig[d0] += B0_SLOTSIZE;

        d0 = _mm256_extract_epi32(v1, 4);
        *(uint64_t*) (ppbig[d0]) = _mm256_extract_epi64(v0, 2);
        ppbig[d0] += B0_SLOTSIZE;
        
        d0 = _mm256_extract_epi32(v1, 6);
        *(uint64_t*) (ppbig[d0]) = _mm256_extract_epi64(v0, 3);
        ppbig[d0] += B0_SLOTSIZE;
    }
    pthread_barrier_wait(ctx+CK_BARRIER);
    rdtsc1 = __rdtsc();
    ...
    if (tid==0) {
        ...
        for (d0=0; d0<B_BUCKETS; d0++) {
            for (d1=0; d1<THREADS; d1++) {
                d3 = d1 * B_BUCKETS + d0;
                q0 += (ppbig[d3]-pvbig);
                dbgctr = ((ppbig[d3]-pvbig) > dbgctr) ? (ppbig[d3]-pvbig) : dbgctr;
                pvbig += B0_SLOTS*B0_SLOTSIZE;
            }
        }
        q0 /= B0_SLOTSIZE;
        dbgctr /= B0_SLOTSIZE;
        printf("BIG0 stat: total: %ld, max: %d\n", q0, dbgctr);
    }
#endif
    ...
    for (uint32_t threadplan=0; threadplan<2; threadplan++) {
        ...
        pvlimit = (bigbucket!=0) ? (ppbig[(THREADS-1)*B_BUCKETS+bigbucket-1] + B0_SLOTSIZE*2) : pvbig0;
        pu64readbig = (pvlimit > pvbig) ? pvlimit : pvbig;
        do {
            d0 = S_BUCKETS*4;
            ...
            q4 = 0;
            for (fromthread=0; fromthread<THREADS*B_BUCKETS; fromthread+=B_BUCKETS) {
                pvlimit = ppbig[fromthread+bigbucket];
    ...
    if (tid==0) {
        ...
        printf("Iteration 1: %u; rdtsc: %ld+%ld\n", edgecount, rdtsc1-rdtsc0, rdtsc2-rdtsc1);
        printf("EDGE1 end: %u, BIG1 start: %u\n", pu32edgesbegin[d1-1], (uint32_t) ((CK_BIG1 - CK_EDGE1)/4));
    }
#endif
    d0 = pu32edges - pu32edgesbegin;
    d1 = (tid == THREADS-1) ? (CK_BIG1-CK_EDGE1)/4 : ((tid+1)*THREADPLAN0*THREADS*B0_SLOTS*B0_SLOTSIZE)/4;
    d0 = (d1 < d0) ? d1 : d0;
    *(--pu32edgei) = d0;
    ...
    // Iteration 2A
    rdtsc0 = __rdtsc();
    pvbig0 = ctx + (CK_BIG1 + B_BUCKETS * THREADS * 8);
    pvbig = pvbig0 + B1_SLOTS * B1_SLOTSIZE * tid;
    ppbig = ctx + CK_BIG1 + B_BUCKETS * 8 * tid;
    for (d1=0; d1<B_BUCKETS; d1++) {
        ppbig[d1] = pvbig;
        pvbig += THREADS*B1_SLOTS*B1_SLOTSIZE;
    }
    ...
    do {
        ...
        *(uint64_t*) (ppbig[d0]) = _mm256_extract_epi64(v0, 3);
        ...
        v0 = _mm256_slli_epi64(_mm256_cvtepu16_epi64(x0), 38);
        q0 = _mm256_extract_epi64(vx, 3);
        vx = _mm256_set_epi64x(q0, q0, q0, q0);
        vx = _mm256_add_epi64(vx, v0);
        SIPHASH;
        //vpayload = _mm256_slli_epi64(_mm256_cvtepu16_epi64(*(__m128i*) pu16rtemp), 21) | vx;
        vpayload = _mm256_slli_epi64(_mm256_cvtepu16_epi64(_mm_set_epi64x(0, *(uint64_t*) pu16rtemp)), 23) | vx;
        pu16rtemp += 4;
        v1 = v0 & yconstbucketmask;
        v0 = _mm256_srli_epi64(v0 & yconstnodemask, B_LOG) | vpayload;
    ...
    *(uint64_t*) (ppbig[d0]) = _mm256_extract_epi64(v0, 3);
    ...
    _Done2A:
    q1 = *(uint64_t*) (ppbig[d0]-B1_SLOTSIZE);
    q1 &= 0xffffffc000000000;
    if (unlikely(q1 != (65536ULL/THREADS-1) << 38)) {
        printf("Error 2A: %lx\n", q1);
        *pu32abort = 1;
    }
    ...
    if (tid==0) {
        ...
        for (d0=0; d0<B_BUCKETS; d0++) {
            for (d1=0; d1<THREADS; d1++) {
                ...
                q0 += (ppbig[d3]-pvbig);
                dbgctr = ((ppbig[d3]-pvbig) > dbgctr) ? (ppbig[d3]-pvbig) : dbgctr;
                pvbig += B1_SLOTS*B1_SLOTSIZE;
            }
        }
        q0 /= B1_SLOTSIZE;
        dbgctr /= B1_SLOTSIZE;
        printf("BIG1 stat: total: %ld, max: %d\n", q0, dbgctr);
    }
#endif
    ...
    bigbucket = tid * BPT;
    bigbucketend = bigbucket + BPT;
    pvbig = pvbig0 + bigbucket * ((uint64_t) THREADS*B1_SLOTS*B1_SLOTSIZE);
    ppbig = ctx + CK_BIG1;
    pvlimit = (bigbucket!=0) ? (ppbig[(THREADS-1)*B_BUCKETS+bigbucket-1] + B1_SLOTSIZE*2) : pvbig0;
    pu64readbig = (pvlimit > pvbig) ? pvlimit : pvbig;
    ...
    TrimEdgeIter4(ctx, ctx+CK_BIG2, ctx+CK_BIG3, tid, 4);
    if (unlikely(*pu32abort))
        goto _GiveUp;
    TrimEdgeIter4(ctx, ctx+CK_BIG3, ctx+CK_BIG2, tid, 5);
    if (unlikely(*pu32abort))
        goto _GiveUp;
    TrimEdgeIter4(ctx, ctx+CK_BIG2, ctx+CK_BIG3, tid, 6);
    if (unlikely(*pu32abort))
        goto _GiveUp;
    
    TrimEdgeIter7(ctx, ctx+CK_BIG3, ctx+CK_BIG2, tid, 7);
        if (unlikely(*pu32abort))
            goto _GiveUp;
            
    for (iter=8; iter<22; iter+=2) {
        TrimEdgeIter8(ctx, ctx+CK_BIG2, ctx+CK_BIG3, tid, iter);
        if (unlikely(*pu32abort))
            goto _GiveUp;
        TrimEdgeIter8(ctx, ctx+CK_BIG3, ctx+CK_BIG2, tid, iter+1);
        if (unlikely(*pu32abort))
            goto _GiveUp;
    }
    
    ramsize0 = TrimRename1(ctx, ctx+CK_BIG2, ctx+CK_BIG3, tid, 22);
    if (unlikely(ramsize0 > 1024))
        goto _GiveUp;
    ramsize1 = TrimRename2(ctx, ctx+CK_BIG3, ctx+CK_BIG2, tid, 23);
    if (unlikely(ramsize1 > 1024))
        goto _GiveUp;
    
    for (iter=24; iter<130; iter+=2) {
    ...
    TrimRename3(ctx, ctx+CK_BIG2, ctx+CK_BIG3, tid, ramsize0, 130);
    d0 = 0;
    for (d1=0; d1<THREADS; d1++) {
        d2 = *(int32_t*) (ctx + CK_THREADRESULT0 + d1*4);
        d0 = (d2>d0) ? d2 : d0;
...
int32_t cuckoo30_mt(tctx_t *ptctx, const uint64_t k0, const uint64_t k1)
{
...
int main(int argc, char *argv[])
{
    ...
    rdtsc0 = __rdtsc();
    solutions = cuckoo30_mt(tctx, k0, k1);
