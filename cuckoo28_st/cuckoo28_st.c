...
static void TrimEdgeIter3(void * const context, void * const dst, void * const src, const uint32_t iter)
{
    ...
    void * const pvdstbody = __builtin_assume_aligned(dst, 64) +  B_BUCKETS * 4;
    void * const pvbighead = __builtin_assume_aligned(src, 64);
    void * const pvbigbody = pvbighead + B_BUCKETS * 4;
    uint8_t * const pu8deg = ctx + CK_PTS1;
    ...
    uint32_t bigbucket, smallbucket, edgecount, dbgctr=0;
    uint32_t d0, d1, d2;
    ...
    pu32big = pvbighead;
    pvbig = pvbigbody;
    pu64readbig = pvbigbody;
    
    for (bigbucket=0; bigbucket<B_BUCKETS; bigbucket++) {
        pu32small = pvsmall0;
        d0 = S_BUCKETS*4;
        do {
            *(pu32small++) = d0;
        ...
        q3 = 0;
        q4 = 0;
        pvlimit = pvbigbody + pu32big[bigbucket];
    ...
    for (d0=0; d0<B_BUCKETS; d0++) {
        edgecount += (pu32dst[d0]-d2);
        dbgctr = ((pu32dst[d0]-d2) > dbgctr) ? (pu32dst[d0]-d2) : dbgctr;
...
static uint32_t TrimRename1(void * const context, void * const dst, void * const src, const uint32_t iter)
{
    void * const ctx = __builtin_assume_aligned(context, 64);
    uint32_t * const pu32dst = __builtin_assume_aligned(dst, 64);
    void * const pvdstbody = __builtin_assume_aligned(dst, 64) +  B_BUCKETS * 4;
    ...
    uint64_t rdtsc0, rdtsc1;
    uint64_t q0, q1, q2, q3, q4;
    uint32_t bigbucket, smallbucket, edgecount, ramsize, dbgctr=0;
    uint32_t d0, d1, d2;
    ...
    d0 = (newnodeid+31) >> B_LOG;  //max address of ram array
...
static uint32_t TrimRename2(void * const context, void * const dst, void * const src, const uint32_t iter)
{
    void * const ctx = __builtin_assume_aligned(context, 64);
    uint32_t * const pu32dst = __builtin_assume_aligned(dst, 64);
    void * const pvdstbody = __builtin_assume_aligned(dst, 64) +  B_BUCKETS * 4;
    ...
    uint64_t *pu64readbig, *pu64readsmall;
    uint32_t *pu32big, *pu32small, *pu32readsmall;
    uint32_t* pu32renamelist = ctx + CK_RENAMELIST;
    void *pvbig, *pvlimit, *pvlimit2, *pvsmall;
    uint64_t rdtsc0, rdtsc1;
    uint64_t q0, q1;
    uint32_t bigbucket, flatbucket, smallbucket, edgecount, ramsize, dbgctr=0;
    uint32_t d0, d1, d2;
    int32_t newnodeid=0, newnodeid_small;
    uint8_t b0;
...
static void TrimEdge1Stage(void * const context, void * const dst, void * const src, const uint32_t ramsize, const uint32_t iter)
{
    void * const ctx = __builtin_assume_aligned(context, 64);
    uint32_t * const pu32dst = __builtin_assume_aligned(dst, 64);
    void * const pvdstbody = __builtin_assume_aligned(dst, 64) +  B_BUCKETS * 4;
    void * const pvsrchead = __builtin_assume_aligned(src, 64);
    void * const pvsrcbody = pvsrchead + B_BUCKETS * 4;
    uint8_t * const pu8deg = ctx + CK_PTS1;
    ...
    uint64_t rdtsc0, rdtsc1;
    uint64_t q0;
    uint32_t srcbucket, edgecount, dbgctr=0;
...
static uint32_t TrimRename3(void * const context, void * const dst, void * const src, const uint32_t ramsize, const uint32_t iter)
{
    void * const ctx = __builtin_assume_aligned(context, 64);
    uint32_t * const pu32dst = __builtin_assume_aligned(dst, 64);
    void * const pvdstbody = __builtin_assume_aligned(dst, 64) +  B_BUCKETS * 4;
    void * const pvsrchead = __builtin_assume_aligned(src, 64);
    void * const pvsrcbody = pvsrchead + B_BUCKETS * 4;
    uint16_t * const pu16deg = ctx + CK_PTS1;
...
static void MakePairs(void * const context, void * const dst, void * const src, const uint32_t ramsize)
{
    void * const ctx = __builtin_assume_aligned(context, 64);
    void * const pvdst = __builtin_assume_aligned(dst, 64);
    void * const pvsrchead = __builtin_assume_aligned(src, 64);
    void * const pvsrcbody = pvsrchead + B_BUCKETS * 4;
    uint32_t * const pu32lastseen = ctx + CK_PTS1;
...
static void MakePairsIndex(void * const context)
{
    ...
    uint32_t * const hashtab = ctx + CK_PTS1;
    uint32_t * const pu32pairsindex = ctx + CK_PAIRSINDEX;
    uint8_t * const pbitfield = ctx + CK_PAIRSBITFIELD;
    
    uint64_t rdtsc0, rdtsc1;
    uint32_t * pu32big = ctx + CK_BIGP;
    ...
    uint32_t paircounter_shifted, bigbucket, pairsindexwrite, dbgctr=0;
    uint8_t b0;
...
static void TrimPairs(void * const context)
{
    ...
    uint64_t rdtsc0, rdtsc1;
    uint32_t d0, d1, d2, d3, d4, innerloop, outerloop, moreiter;
    ...
    for (outerloop=0; outerloop<24; outerloop++) {
        ...
        pu32end = &pu32pairsindex[pu32pairsindex[6]];
...
static void RecoverSolution(void * const context, uint32_t * listv, uint32_t halfcyclelen)
{
    ...
    uint32_t * const pu32edges = pu32edgei + B_BUCKETS * S_BUCKETS;
    void *pvedges, *pvend;
    uint32_t *solbuf;
    ...
    for (cyclepos=0; cyclepos<halfcyclelen; cyclepos++) {
        ...
        d2 = pu32renamelist[d0];
        d4 = ((d2 >> 19) & 0xff);
        sipvnode = ((d2 << 13) & 0x07ffe000) | ((d2 >> 6) & 0x00001f00) | d4;
        flatbucket = ((sipvnode >> 8) & 0xff) | (d4 << 8);
//printf("%08x %08x %08x %08x\n", listv[cyclepos], d0, d2, sipvnode);
...
int32_t cuckoo28_st(void *context, const uint64_t k0, const uint64_t k1)
{
    __m256i in0,in1,in2,in3,in1a;
    ...
    uint32_t bigbucket, smallbucket, iter, ramsize0, ramsize1;
    ...
    *(uint32_t *) (ctx + CK_ABORT) = 0;
    ...
    pu32edges = pu32edgei + (B_BUCKETS * S_BUCKETS);
    ...
    for (bigbucket=0; bigbucket<B_BUCKETS; bigbucket++) {
        ...
        q3 = 0;
        q4 = 0;
        pvlimit = pvbig0 + pu32big[bigbucket];
    ...
    *(--pu32edgei) = d0;
    ...
    // Iteration 2A
    rdtsc0 = __rdtsc();
    pvbig0 = ctx + (CK_BIG1 + B_BUCKETS * 4);
    pu32big = ctx + CK_BIG1;
    d0 = 0;
    for (d1=0; d1<B_BUCKETS; d1++) {
        pu32big[d1] = d0;
    ...
    *(uint64_t*) (pvbig0+d1) = _mm256_extract_epi64(v0, 3);
    ...
    pvdst0 = ctx + (CK_BIG2 + B_BUCKETS * 4);
    pu32dst = ctx + CK_BIG2;
    d0 = 0;
    for (d1=0; d1<B_BUCKETS; d1++) {
        pu32dst[d1] = d0;
    ...
    ramsize0 = TrimRename1(ctx, ctx+CK_BIG2, ctx+CK_BIG3, 4);
    if (unlikely(ramsize0 > 1024))
        goto _GiveUp;
    ramsize1 = TrimRename2(ctx, ctx+CK_BIG3, ctx+CK_BIG2, 5);
    if (unlikely(ramsize1 > 1024))
        goto _GiveUp;
    
    for (iter=6; iter<30; iter+=2) {
        TrimEdge1Stage(ctx, ctx+CK_BIG2, ctx+CK_BIG3, ramsize0, iter);
        TrimEdge1Stage(ctx, ctx+CK_BIG3, ctx+CK_BIG2, ramsize1, iter+1);
    }
    d0 = TrimRename3(ctx, ctx+CK_BIG2, ctx+CK_BIG3, ramsize0, 30);
    if (unlikely(d0>262112))
        goto _GiveUp;
    MakePairs(ctx, ctx+CK_PAIRSLIST, ctx+CK_BIG2, ramsize1);
    MakePairsIndex(ctx);
...
int main(int argc, char *argv[])
{
    ...
    rdtsc0 = __rdtsc();
    solutions = cuckoo28_st(ctx, k0, k1);
