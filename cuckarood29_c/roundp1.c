...
//B 0  1d 0
//C 1  1  2d
void rounda1(void *context, uint32_t tid)
{
        ...
        for (int fromthread=0; fromthread<THREADS; fromthread++) {
            ...
            do {
                ...
                *(uint64_t *) ((void *) pu32DstCtr + rel32Write) = u64ReadVal;
                uint32_t u32tmp = pu8Bitmap[(uint32_t) u64ReadVal >> 7];
                u32tmp = (u32tmp + 1) & 2;
                pu8Bitmap[(uint32_t) u64ReadVal >> 7] = (uint8_t) u32tmp;
...
void roundb1(void *context, uint32_t tid)
{
    void *const ctx = __builtin_assume_aligned(context, 32768);
    uint32_t *const pu32DstCtr = ctx + (tid * CK_THREADSEP) + CK_COUNTER1;
    uint32_t *pu32SrcCtr = ctx + CK_COUNTER0;
...
void roundc1(void *context, uint32_t tid)
{
    void *const ctx = __builtin_assume_aligned(context, 32768);
    uint32_t *const pu32DstCtr = ctx + (tid * CK_THREADSEP) + CK_COUNTER1;
    uint32_t *pu32SrcCtr = ctx + CK_COUNTER0;
