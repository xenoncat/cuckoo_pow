...
void round3(void *context, uint32_t tid)
{
    ...
    uint32_t rel32ReadStart = CK_BUCKET2 - CK_COUNTER0 + i32SrcBucket * CK_BUCKETSIZE2;
    ...
    uint32_t rel32CtrInit = CK_BUCKET3 - CK_COUNTER1;
    for (int i=0; i<2048; i++) {
        pu32DstCtr[i] = rel32CtrInit;
        rel32CtrInit += CK_BUCKETSIZE3;
    }
    ...
    do {
        ...
        for (int fromthread=0; fromthread<THREADS; fromthread++) {
            ...
            do {
                ...
                pRead += 5;
                u32tmp = (pu32Bitmap[(u64ReadVal & 0x1fff) - 0x2000] >> ((u64ReadVal >> 13) & 0x1f)) & 1;
