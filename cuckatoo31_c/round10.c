...
void round10(void *context, uint32_t tid)
{
    ...
    uint32_t rel32ReadStart = CK_BUCKET9 - CK_COUNTER1 + CK_BUCKETSIZE9 + i32SrcBucket * CK_BUCKETSIZE9;
    ...
    uint32_t rel32CtrInit = CK_BUCKET8 - CK_COUNTER0;
    for (int i=0; i<8192; i++) {
        pu32DstCtr[i] = rel32CtrInit;
        rel32CtrInit += CK_BUCKETSIZE8;
    }
    ...
    do {
        ...
        for (uint32_t fromthread=0; fromthread<8192; fromthread+=(8192/THREADS)) {
            ...
            do {
                ...
                pRead += 6;
                uint32_t u32tmp = (pu32Bitmap[(u64ReadVal & 0x1fff) - 0x2000] >> ((u64ReadVal >> 13) & 0x1f)) & 1;
        ...
        for (uint32_t fromthread=0; fromthread<8192; fromthread+=(8192/THREADS)) {
            ...
            do {
                uint64_t u64ReadVal = *(uint64_t *) pRead;
                pRead += 6;
                uint32_t u32tmp = (pu32Bitmap[(u64ReadVal & 0x1fff)] >> ((u64ReadVal >> 13) & 0x1f)) & 1;
                u32Bucket = ((u64ReadVal >> 36) & 0xfff) | fromthread;
                uint32_t rel32Write = pu32DstCtr[u32Bucket];
                *(uint64_t *) ((void *) pu32DstCtr + rel32Write) = (u64ReadVal & u64ItemMaskAnd) | u64ItemMaskOr;
                pu32DstCtr[u32Bucket] = rel32Write + (u32tmp * 6);
