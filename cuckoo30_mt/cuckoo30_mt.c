...
//2, 2, 20, 42 (nonce=11)
static uint64_t k0def = 0x1e0b3099cd3c2dea;
static uint64_t k1def = 0xc98d494b02729146;
...
static void TrimEdge2Stage(void * const context, void * const dst, void * const src, const uint32_t tid, const uint32_t iter)
{
    void * const ctx = __builtin_assume_aligned(context, 64);
    uint32_t * const pu32dst = __builtin_assume_aligned(dst, 64) + B_BUCKETS * 4 * tid;
    void * const pvdstbody = __builtin_assume_aligned(dst, 64) +  B_BUCKETS * THREADS * 4;
    ...
    for (; bigbucket<bigbucketend; bigbucket++) {
        ...
        if (unlikely(q4 != 0x3ff00000000)) {
            printf("Error TrimEdge2Stage %lx\n", q4);
            *pu32abort = 1;
            goto _GiveUpT2S;
        }
        pvsmall = pvsmall0 + S_BUCKETS*4;
        ...
        for (smallbucket=0; smallbucket<S_BUCKETS; smallbucket++) {
            ...
            if (unlikely(d3 != 0xc0)) {
                printf("Error TrimEdge2StageSmall %x\n", d3);
                *pu32abort = 1;
                goto _GiveUpT2S;
            }
            pvsmall += S1_SLOTS*S1_SLOTSIZE;    //advance to the next bucket
    ...
    for (; bigbucket<bigbucketend; bigbucket++) {
        ...
        for (fromthread=0; fromthread<THREADS*B_BUCKETS; fromthread+=B_BUCKETS) {
            ...
            while ((void*) pu64readbig < pvlimit2) {
                ...
                d0 = q0 & S_BUCKETMASK;
                d1 = pu32small[d0];
                *(uint64_t*) (pvsmall0+d1) = (q0 >> S_LOG); //Mask 0x0fffffffff omitted
                pu32small[d0] += S1_SLOTSIZE;
...
static void RecoverSolution(void * const context, uint32_t * listv, uint32_t halfcyclelen)
{
    ...
    for (cyclepos=0; cyclepos<halfcyclelen; cyclepos++) {
        ...
        sipvnode = ((d2 << 14) & 0x1fffc000) | ((d2 >> 7) & 0x00003f00) | d4;
...
void* worker(void *tctx)
{
    ...
    *(--pu32edgei) = d0;
    ...
    do {
        ...
        *(uint64_t*) (pvbig0+pu32big[d0]) = _mm256_extract_epi64(v0, 3);
        ...
        v0 = _mm256_slli_epi64(_mm256_cvtepu16_epi64(x0), 34);
        ...
        vpayload = _mm256_slli_epi64(_mm256_cvtepu16_epi64(_mm_set_epi64x(0, *(uint64_t*) pu16rtemp)), 21) | vx;
        pu16rtemp += 4;
        v1 = v0 & yconstbucketmask;
        v0 = _mm256_srli_epi64(v0 & yconstnodemask, B_LOG) | vpayload;
    ...
    *(uint64_t*) (pvbig0+d1) = _mm256_extract_epi64(v0, 3);
    ...
    q1 &= 0xfffffffc00000000;
    if (unlikely(q1 != (65536ULL/THREADS-1) << 34)) {
        printf("Error 2A: %lx\n", q1);
        *pu32abort = 1;
    }
    ...
    for (iter=4; iter<10; iter+=2) {
        TrimEdge2Stage(ctx, ctx+CK_BIG2, ctx+CK_BIG3, tid, iter);
        if (unlikely(*pu32abort))
            goto _GiveUp;
        TrimEdge2Stage(ctx, ctx+CK_BIG3, ctx+CK_BIG2, tid, iter+1);
        if (unlikely(*pu32abort))
            goto _GiveUp;
    }
    
    ramsize0 = TrimRename1(ctx, ctx+CK_BIG2, ctx+CK_BIG3, tid, 10);
    if (unlikely(ramsize0 > 1024))
        goto _GiveUp;
    ramsize1 = TrimRename2(ctx, ctx+CK_BIG3, ctx+CK_BIG2, tid, 11);
    if (unlikely(ramsize1 > 1024))
        goto _GiveUp;
    
    for (iter=12; iter<62; iter+=2) {
    ...
    TrimRename3(ctx, ctx+CK_BIG2, ctx+CK_BIG3, tid, ramsize0, 62);
    d0 = 0;
    for (d1=0; d1<THREADS; d1++) {
        d2 = *(int32_t*) (ctx + CK_THREADRESULT0 + d1*4);
        d0 = (d2>d0) ? d2 : d0;
